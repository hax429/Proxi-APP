  ...
  if (deviceCount == 1) {
    ...
    if (!uwbInitialized) {
      if (UWB.begin()) {
        ...
        Serial.println("ERROR: Failed to initialize UWB stack");
      }
    }
  }
...
void setup() {
  ...
  if (UWBNearbySessionManager.begin("Gabriel's Pilot")) {
    Serial.println("BLE services initialized successfully");
    Serial.println("Advertising as 'Gabriel's Pilot'");
  } else {
    Serial.println("ERROR: Failed to initialize BLE services");
  }
